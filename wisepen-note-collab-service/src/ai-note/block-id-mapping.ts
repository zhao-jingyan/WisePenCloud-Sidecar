...
export interface BlockIdMapping {
  version: string;
...
function visitBlocks(blocks: NativeBlock[], visitor: (block: NativeBlock) => void): void {
...
function createMapping(snapshot: NoteSnapshot): BlockIdMapping {
  const internalToPublic = new Map<string, string>();
  const publicToInternal = new Map<string, string>();
  visitBlocks(snapshot.blocks, (block) => {
  ...
  return { version: snapshot.version, internalToPublic, publicToInternal };
}
...
export function rememberBlockIdMapping(
  ...
  const existing = cache.byVersion.get(snapshot.version);
  if (existing) {
    cache.byVersion.delete(snapshot.version);
    cache.byVersion.set(snapshot.version, existing);
    cache.latestVersion = snapshot.version;
    return existing;
  }

  const mapping = createMapping(snapshot);
  cache.byVersion.set(snapshot.version, mapping);
  cache.latestVersion = snapshot.version;
...
export function getPublicBlockId(mapping: BlockIdMapping, internalId: string): string {
  const publicId = mapping.internalToPublic.get(internalId);
...
function operationBlockIds(operation: PatchOperation): string[] {
