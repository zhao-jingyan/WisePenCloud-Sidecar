...
import { isValidPatchContentShape } from './patch-content-codec';
...
function parseOperation(value: unknown, index: number): PatchOperation {
  ...
  if (kind === 'replaceContent') {
    if (!isValidPatchContentShape(value.content)) {
  ...
  if (kind === 'insertBlock') {
    ...
    if (!isValidPatchContentShape(value.block.content)) {
